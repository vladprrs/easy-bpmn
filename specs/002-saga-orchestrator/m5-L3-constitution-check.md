...
Three design choices are worth recording against the Development-Workflow gate as **deliberate v1 scope
...
All three narrowings are **v1 scope decisions carved out at layer-design time**, not violations of an
existing constitutional guarantee — no rejected simpler alternative applies to any of them beyond what the
table above already records; they are recorded here per the Development-Workflow gate's Complexity Tracking
convention (mirroring the M5-L1 and M5-L2 precedent) because a future reader of the decomposition doc alone
could otherwise expect richer MI-subProcess bodies, MI-as-a-unit compensation, or non-`All` completion
behavior in v1.

Result: **PASS** — no unresolved violation; all three narrowings are scope-bound v1 decisions made at the
