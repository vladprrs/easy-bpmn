...
function Node({ name, value, depth }: { name?: string; value: unknown; depth: number }) {
  ...
  if (isR2Ref(value)) {
    return (
      <div className="font-mono text-xs">
        ...
        <span className="italic text-slate-500">large payload stored in R2 ({value.__r2})</span>
      </div>
    );
  }
  ...
  return (
    <div className="font-mono text-xs">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-1 text-slate-400 hover:text-slate-200">
        <ChevronRight className={`h-3 w-3 transition-transform ${open ? "rotate-90" : ""}`} />
        {name ?? (Array.isArray(value) ? "[ ]" : "{ }")}
        <span className="text-slate-600">{Array.isArray(value) ? `(${entries.length})` : ""}</span>
      </button>
      {open && (
        <div className="ml-3 border-l border-ink-700 pl-3">
          {entries.length === 0 && <span className="text-slate-600">empty</span>}
          {entries.map(([k, v]) => (
...
export function JsonView({ value }: { value: unknown }) {
  if (value === undefined || value === null || (typeof value === "object" && Object.keys(value).length === 0)) {
    return <div className="p-2 text-xs text-slate-600">no data</div>;
  }
  return (
    <div className="overflow-auto rounded bg-ink-900 p-2">
