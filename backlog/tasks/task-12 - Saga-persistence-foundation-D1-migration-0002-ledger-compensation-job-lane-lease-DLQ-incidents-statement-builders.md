...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 migrations/0002_saga.sql applies cleanly on top of 0001 in the vitest workerd runtime (applyD1Migrations via tests/apply-migrations.ts) with no error; statements use IF EXISTS / IF NOT EXISTS guards so re-application is safe.
- [x] #2 service_task_jobs gains is_compensation (NOT NULL DEFAULT 0), compensates_element_id, worker_id, lock_token, lock_expires_at, activation_expires_at, error_code; existing forward-job inserts continue to succeed without supplying the new columns.
- [x] #3 The single-column job uniqueness is removed and replaced by (instance_id, element_id, is_compensation): two FORWARD jobs for the same (instance_id, element_id) are rejected, while a forward job plus a compensation job (is_compensation=1) for that same element are BOTH accepted.
...
- [x] #5 Inserting a duplicate completed step for the same (instance_id, element_id) via INSERT OR IGNORE leaves exactly one row (no double-compensation), asserted by test.
- [x] #6 idx_jobs_leasable (task_type, status, lock_expires_at) and idx_instances_workspace_status (workspace_id, status) both exist.
- [x] #7 incidents gains kind and resolution columns; the existing incidentStmt insert still succeeds and getIncidentForInstance returns the new fields.
- [x] #8 worker_credentials table exists with workspace_id, token_hash, created_at, revoked_at (token_hash uniquely resolvable to a workspace).
- [x] #9 Persistence statement builders are updated and compile under tsc: JobRow + createJob carry the new job columns (forward defaults: is_compensation=0, compensates_element_id NULL) with optional params so existing callers are unchanged; a new saga ledger module exposes INSERT-OR-IGNORE, a reverse-order (seq DESC) scope select filtered to compensation_status IN ('pending','compensating','failed'), and a compensation-status update; incident builders carry kind/resolution.
...
- [x] #11 Docs: specs/002-saga-orchestrator/data-model.md records the new entities/columns/indexes, and the migration header documents the §5 'relaxed index not stable past M1' caveat.
<!-- AC:END -->
...
Migration `migrations/0002_saga.sql` (additive over 0001, applied by the vitest workerd runtime): service_task_jobs gains workspace_id, is_compensation (NOT NULL DEFAULT 0), compensates_element_id, worker_id, lock_token, lock_expires_at, activation_expires_at, error_code; dropped uq_jobs_instance_element → uq_jobs_instance_element_kind (instance_id, element_id, is_compensation) + idx_jobs_leasable; created saga_steps ledger (§5 columns) + uq_saga_steps_forward + idx_saga_steps_scope; incidents gains kind + resolution; idx_instances_workspace_status; worker_credentials table (+ uq token hash + workspace idx). Header carries the §5 "relaxed index not stable past M1" caveat. Persistence: new src/persistence/saga.ts (insertSagaStepStmt INSERT-OR-IGNORE with atomic monotonic seq, selectScopeStepsForCompensation seq DESC filtered to pending/compensating/failed, attachCompensationJobStmt, updateCompensationStatusStmt, getSagaStep(s)); new src/persistence/worker-credentials.ts; instances.ts JobRow + createJob/createJobStmt extended with the new columns via optional params (forward defaults unchanged), incidentStmt + getIncidentForInstance carry kind/resolution + setIncidentResolution; contracts Incident type gains kind/resolution. Test tests/integration/migration-0002-saga.test.ts asserts all new tables/columns/indexes via PRAGMA, the two job-uniqueness behaviors (forward+comp allowed, two forwards rejected), and saga_steps INSERT-OR-IGNORE idempotency. specs/002-saga-orchestrator/data-model.md records the entities (written by TASK-8). Consolidated worker_credentials into 0002_saga.sql (not a separate 0002 file) to avoid a numeric-prefix collision. typecheck + full suite green.
