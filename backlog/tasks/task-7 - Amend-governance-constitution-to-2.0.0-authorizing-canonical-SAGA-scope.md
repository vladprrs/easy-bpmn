...
Amended `.specify/memory/constitution.md` 1.0.0 → 2.0.0 (MAJOR; scope expansion invalidating prior governance). Rewrote the Sync Impact Report with the bump rationale and the enumerated diff. Widened **Principle I → "Standard BPMN Profile Only"** to list the canonical-saga construct set (bpmn:transaction, compensate/error/cancel boundaryEvent, isForCompensation handler, bpmn:association, cancel endEvent, bpmn:error) while preserving the no-custom-notation / XSD-valid / modeler-round-trippable / reject-unsupported-flow-node-with-element-id+reason clause and the tolerate-ignorable-content clause. Trimmed the MVP Scope exclusion list to remove only the M1-shipped constructs (transaction subprocess, compensation, saga boundary events), keeping gateways/conditional flows, timers/other event defs, non-transaction subprocess/callActivity/adHoc, multi-instance, user-task/forms, migration, full Camunda/Zeebe compat, and visual modeler excluded. Added **Principle VI — SAGA / Compensation Integrity** (reverse completion order; idempotent + at-least-once; transaction-scoped; receives original input + captured output; triggered only by transaction Cancel never by an uncaught Error/Hazard; compensator exhaustion → deterministic `compensationFailed` + operator remediation; audited). Bumped footer to 2.0.0, Last Amended 2026-06-08. Propagated to all three `.specify/templates/*` (plan Constitution Check: widened BPMN gate + new SAGA gate; spec BPMN/SAGA impact prompts; tasks constitution-critical test list adds compensation ordering, saga state transitions, worker auth/isolation, operator remediation). No placeholder tokens remain. Constitution gate: N/A (governance/docs-only, no runtime/API/persistence change) — verified by grep (version 2.0.0, exclusion list free of removed items, no [TODO]/[PLACEHOLDER]).
