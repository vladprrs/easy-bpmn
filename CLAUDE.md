...
- `MAX_ELEMENT_OCCURRENCES = 1000` (in `src/runtime/engine.ts`) caps visits per element; exceeding it
  ...
  = a process with no `callActivity`). `npm run check:docs` enforces that every copy of each of these
...
- **Reject unsupported *flow nodes* before publish, with element id + reason** (complex gateways, non-block-structured (non-SESE) parallel/inclusive
  ...
  `multiInstanceLoopCharacteristics`, conditional/default flows not leaving an
  ...
  call tree (a v1 narrowing) — `multiInstanceLoopCharacteristics`, escalation, signal, and the
