...
Two-stage review done. Spec review: compliant, incl. an end-to-end probe of retry-after-noPath (operator /retry from a noPath Hazard re-evaluates FRESH — no decision row was written for the failed visit; with patched variables the saga proceeds; without, a bounded fresh noPath incident). Quality review: Ready to merge (Yes) + polish in caf2ed5: retry-after-noPath pinned by integration test (+ comment at the resetJobForRetry call site — 0-row match on non-task elements is by design); stale TASK-34 comments fixed; normalizeFeelValue moved to expressions.ts as THE canonical contract (non-finite numbers -> String(value), not a tag — doc now truthful); evaluationsAsJson helper kills the double-casts; payloadByteSize hoisted; null condition on a non-default multi-out flow now throws a loud invariant.

Design decisions: pass-through (1-out XOR) writes a uniform decision row (evaluations [], snapshot null) — it IS the per-occurrence rewalk predicate cycles need; evaluations record only actually-evaluated flows (trace, not flow list); noPath writes NO decision row (the incident is the record; operator retry re-evaluates fresh — the only non-bricking semantics); hard-FEEL-error incident uses kind=serviceTaskFailure with a distinguishing reason (dedicated conditionFailure kind deferred to M3); decideGateway exported solely for the deterministic duplicate-walk race test.

Carried into TASK-35: decide MAX_ELEMENT_OCCURRENCES=1000 against the Cloudflare Workflows TOTAL step budget (a hot gateway cycle in workflow mode could exhaust the platform budget before loopLimit fires — ~2 steps per gateway visit). Carried into TASK-37: design-doc one-liner for the variablesSnapshotOmitted flag; retry-after-noPath semantics doc note. Post-M2 chore: engine.ts at 1380 lines — extract gateway/compensation/receive sections when next touched.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
exclusiveGateway dispatch landed in the engine: one persisted step gw:el#occ per visit; fast-forward predicate = gateway_decisions row existence (never re-evaluate, zero writes); else document-order short-circuit FEEL evaluation (first boolean true wins) -> default -> terminal noPath incident (Hazard in transaction: no auto-compensation, /cancel compensates — full chain tested; operator /retry re-evaluates fresh, pinned). Decision row (plain INSERT; UNIQUE violation -> re-read + follow recorded, non-swallowing catch) + applyTransition + gatewayDecisionEvaluated history ({chosenFlowId, occurrence, isDefault, evaluations[{flowId, expression, result, value, warnings}]}) in ONE dbBatch. Pass-through 1-out gateways write uniform decision rows (cycle-correct, conditions never evaluated). FEEL values JSON-normalized via canonical normalizeFeelValue in expressions.ts; variables_snapshot payload-capped -> NULL + variablesSnapshotOmitted flag. Hard FEEL error -> deterministic incident distinguishable from noPath. LOOP_XOR_BPMN publishes and executes end-to-end (2 iterations, exits via default). Replay pinned in BOTH modes (direct rewalk with mutated variables; memoizing-harness crash-after-commit). gw-guard arm + test replaced; MARKER wordings fixed. Tests 226/226 ×2 (13 new + retry pin − guard test). Commits 5846495 + caf2ed5.
