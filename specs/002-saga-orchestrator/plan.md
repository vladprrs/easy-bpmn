...
branch jobs leasable at once); the engine serializes drives. The original L3 design used a `Promise.race`
multi-wait within the single Workflow; this was **superseded by a single replay-stable `bpmn_wake` wake
with re-walk-from-D1 (TASK-54)**, now the live concurrency strategy — see `quickstart.md` ("Fix (shipped,
TASK-54)").
